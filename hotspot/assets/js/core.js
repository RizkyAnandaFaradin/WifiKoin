var errorCodeMap = [];
errorCodeMap['coins.wait.expired'] = 'Waktu Habis, Silahkan Coba Kembali';
errorCodeMap['coin.not.inserted'] = 'Koin Gagal Masuk';
errorCodeMap['coinslot.cancelled'] = 'Mesin Wifi Koin Membatalkan Transaksi Anda';
errorCodeMap['coinslot.busy'] = 'Mesin Wifi Koin Sedang digunakan';
errorCodeMap['coin.slot.banned'] = 'Mohon Maaf Untuk Sementara Kamu Tidak Bisa Menggunakan Wifi Koin, Karena Terlalu Banyak Upaya Memasukan Koin';
errorCodeMap['coin.slot.notavailable'] = 'Mesin Wifi Koin Tidak Terhubung ke Jaringan';
errorCodeMap['no.internet.detected'] = 'Mesin Wifi Koin Tidak Mendapatkan Akses Internet';
...
$(document).ready(function () {
  ...
  if (loginError != '' && voucher != null && voucher != '') {
    ...
    $.toast({
      title: 'Error',
      content: 'Kode Yang Kamu Masukan Salah',
      type: 'error',
      delay: 5000,
    });
  }
  ...
  if (AutoScreenshot) {
    document.querySelector('#saveVoucherButton').addEventListener('click', function () {
      html2canvas(document.querySelector('.inscoin'), {
    ...
    document.querySelector('#pauseTimeBtn').addEventListener('click', function () {
      html2canvas(document.querySelector('.info-status'), {
        onrendered: function (canvas) {
          return Canvas2Image.saveAsPNG(canvas);
        },
      });
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (data.status == 'true') {
        ...
        if (isMultiVendo) {
          $('#insertCoinModalTitle').html('SIlahkan Masukan Koin ' + $('#vendoSelected option:selected').text());
        }
        insertcoinbg.play();
      } else {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (data.status == 'true') {
        ...
        $.toast({
          title: 'Success',
          content: 'Terimakasih, Koin Kamu Sudah di Terima, Mohon Tunggu Sebentar',
          type: 'success',
          delay: 3000,
        });
...
function notifyCoinSuccess(coin) {
  $.toast({
    title: 'Koin Diterima',
    content: coin + ' Koin Rp.500- Diterima',
    type: 'success',
    delay: 2000,
  });
  coinCount.play();
